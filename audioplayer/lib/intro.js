...
module.exports = function (opt = {}, cb = noop) {
  ...
  if (isMobile) {
    intro2.innerHTML = "Hold the button below to load a new track";
    intro3.innerHTML = "Release the button to play";
  }
  ...
  function animateIn(element, opt = {}, cb = noop) {
    ...
    const duration =
      typeof opt.duration === "number" ? opt.duration : globalDuration;
    const children = getAnimatables(element, opt);
    children.forEach((child, i) => {
      const tween = { opacity: 0, yOff, element: child };
      update({ target: tween });
      const lastTween = tweenr
        .to(tween, { delay, opacity: 1, duration, ease: "quadOut" })
        .on("update", update);
      tweenr.to(tween, {
        delay,
        yOff: 0,
  ...
  function getAnimatables(element, opt = {}) {
    const children = Array.prototype.slice.call(
      element.querySelectorAll(opt.childTagName || "p")
    );
