...
module.exports = function ({
  ...
  const materials = [wireMat, plainMat, shaderMat, shaderMatWire];
  ...
  loop.on("tick", (dt) => {
    ...
    meshes.forEach((m) => {
      if (m.material.uniforms) {
        m.material.uniforms.aspect.value =
  ...
  function nextGeometry(opt = {}) {
    for (
      let i = 0, count = 0;
      i < meshes.length && count < ADDITIONAL_PARTS;
      i++
    ) {
      const m = meshes[i];
      ...
      if (
        !m.active &&
        (opt.type === m.type || typeof opt.type === "undefined")
      ) {
        resetMesh(m);
        count++;
      }
  ...
  function resetMesh(mesh, opt = {}) {
    ...
    if (mesh.material.uniforms) {
      mesh.material.uniforms.dance.value =
        Math.random() > 0.5 ? random(0, 1) : 0;
    }
    setRandColor(mesh);
    if (opt.animate !== false) {
      ...
      tweenr
        .to(tween, { duration: 0.5, value: 1, ease: "expoOut" })
        .on("update", () => {
